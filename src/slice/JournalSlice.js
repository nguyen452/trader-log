...
export const fetchJournalPageData = createAsyncThunk(
    ...
    async (_, thunkAPI) => {
        ...
        const uriEncodedDates = paginatedDates
            .map((date) => `date=${encodeURIComponent(date)}`)
            .join("&");

        const response2 = await fetch(
            `http://localhost:4000/api/journal/tradesData?${uriEncodedDates}`,
            {
                method: "Get",
                credentials: "include",
            }
...
export const createJournalEntry = createAsyncThunk(
    ...
    async ({ date, entry, hasTrade }) => {
        const response = await fetch(
            "http://localhost:4000/api/journal/entry",
            {
                method: "POST",
                credentials: "include",
                headers: {
...
export const selectErrorMessage = (state) => state.journal.errorMessage;
